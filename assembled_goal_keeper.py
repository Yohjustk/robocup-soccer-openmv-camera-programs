...
while(True):
    ...
    if wall == 0:
        ...
        radial_result = radial_checker(360,0,180,11,30,120,my_goal_threshold,0) #(スキャン終了角度、スキャン終了角度、スキャン中心角度、角度分割数、スキャン開始半径、スキャン終了半径、閾値、モード｛０＝自ゴール距離　１＝敵スキャン｝）
        #print(radial_result)
        if radial_result == 0:
            continue
        elif radial_result == 1:#ゴールが遠くなかった→ボールを探す
               #goal_cordinate1 = [] #ゴール探索結果用リスト初期化
